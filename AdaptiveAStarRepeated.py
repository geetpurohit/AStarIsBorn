...
def ComputePath(openHeap, closedHeap, goalState, expandedStates, counter, states):
    while goalState.g > openHeap.peek().f:
        ...
        for action in actionList:
            ...
            if searchedState.search < counter:
                searchedState.g = 99999
                searchedState.search = counter
            if searchedState.g > minState.g + 1:
                searchedState.g = minState.g + 1  # Update the cost
...
# main function
def repeatedAdaptiveAStar(states, startLocation, goalLocation, isLargerGFirst: bool, totaltimestep, totalactualcost, totalnumberofastarinteractions,totaltimecsost, totalnumberofexpandedcells,numMazeNoPath):
    ...
    while startState != goalState:
        ...
        startState.g = 0  # record cost for start state to reach start state, which is 0
        startState.search = counter  #
        goalState.g = 99999  # record cost for goal state to reach start state, which uses 999 as infinity
        ...
        while not closedHeap.isEmpty():
            ...
            tempState.h = goalState.g - tempState.g
